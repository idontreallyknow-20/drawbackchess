...
| A16 | **Every search budget in the engine is worth up to twice what its caller asked for.** `negamax` aborts at `budget * 2` (`ai.ts:615`) and the iterative-deepening loop only checks the clock AFTER a whole depth completes (`ai.ts:440`), so a depth that starts just under the budget runs on. Measured, not inferred: `analyzeBoard(board, 300)` on `/analysis` was a **601ms** main-thread block after every move. This is not only an analysis problem. `aiBudgetMs` exists so "the bot spends time like a human player and can never think its whole bank away in fast time controls" (its own comment), and it clamps to `min(base, remainingClock / 10)`; if the real spend is up to 2x, that guarantee is 2x looser than it reads, and in a 1+0 game near the floor the bot can spend 120ms of a 60ms budget. Either make the abort `budget` and accept losing a partial depth, or rename the parameter to what it is and fix the call sites that assume otherwise. Found while measuring the eval bar's frame cost | S | TODO |
| A17 | **`/game/[id]` can strand a viewer on "Connecting..." forever.** `spectate()`'s catch block is an unawaited async function, so anything that throws inside it (`isArenaGameLive`, for one) is swallowed and the page never leaves `{kind: "loading"}`. Hit repeatedly while trying to reach the replay view. Note the connecting branch now has an `sr-only` h1 (round 7), so the page is named while it hangs, which makes the hang quieter rather than better | S | TODO |
| A18 | **Move the eval search into a worker.** `src/workers/aiWorker.ts` already proves the pattern; an `evalWorker` taking a bare `BoardState` and returning `{move, scoreCp, depth}` removes the frame-cost problem entirely and would let `/analysis` run a real 300ms budget at zero blocking. Everything shipped in round 8 (an idle-callback ladder, cancelled on position change, suspended on a hidden tab, engine `import()`ed lazily) is a main-thread mitigation of this | M | TODO |
...
| C45 | **The rarity chips fail AA in all three themes, not just light, and they cannot be fixed from CSS.** `src/lib/achievementTheme.ts` sets `color` as an INLINE STYLE from literal hexes, and the locked chips also carry `opacity: 0.7` which earlier numbers did not fold in. With it folded in, measured on `/achievements`: light Common **1.44** (25 elements), Legendary **1.39** (20), Rare **1.66** (28), Epic **2.13** (29); dark Epic 2.81, Rare 3.84; midnight Epic 2.90, Rare 3.97. The shape that unblocks it: make `color` a `var(--rarity-ink-<r>, <current>)` so `globals.css` can own it per theme. Solved values keeping the 0.7 opacity (hue and chroma pinned, same method as the tiers): dark and midnight rare `#6ed5c1`, epic `#d7b0ff`; light common `#3d382e`, rare `#00433a`, epic `#54217c`, legendary `#4b3600`. Better still, replace the `opacity: 0.7` with a dimmer colour, since opacity multiplies every failure, and then light needs only `#787367` / `#008070` / `#8a5ab8` / `#916c00` | S | TODO |
...
| C47 | **`GlossaryTerm.tsx` renders `span[role="button"] tabIndex={0}` inside card `<button>`s.** Nested interactive semantics, invalid HTML, and the root cause of the draft-card dead zones fixed from the outside in round 8 (a capture-phase pick on the wrapper). A proper fix makes the card face not a `<button>`, or the term not interactive on picker surfaces | S | TODO |
| C48 | **`.rule-ornament` uppercases the tier label, so cards announce "TRIVIAL" and "BRUTAL" in caps** (Chrome applies `text-transform` when computing accessible names). Same ornament section 3 already retires for section labels, so this is C-something and a sentence-case fix at once | XS | TODO |
| C49 | **400 to 1200ms from hydration to the draft overlay mounting**, before any of the round-8 choreography starts. It is now the largest single block in the opening-pick arc and nobody owns it. `src/app/game/page.tsx` | S | TODO |
