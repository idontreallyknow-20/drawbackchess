...
| A14 | **The parser cannot read DELAYED or CONDITIONAL removal, and that one shape accounts for most of A12.** The pattern: the piece nouns sit in one clause and the removing verb in a later one, with an anaphor in between. "Mark up to three enemy knights, bishops, or pawns; **after your opponent's next move**, lightning falls and removes **each marked piece** that still stands." "Roll a powder keg onto an empty square ... **when it blows**, every enemy piece except the king ...". "Your **next three captures each** detonate one square in every direction, destroying up to two adjacent enemy pieces." The parser wants a removal verb near a piece noun; here the verb's object is "each marked piece" / "them" / "it", and the antecedent is a sentence away. It already has the machinery for this: the `pending` mechanism binds an unscored group to a later clause when there is exactly one candidate, and it deliberately refuses when there are two. Extending it across a clause or sentence boundary for an ANAPHORIC object is the fix. Prove the blast radius the same way round 8 proved the replacement and gloss passes: diff every card's M before and after and expect only the intended ones to move (that check found exactly 2 of 1665 for those two passes). One warning from a round-8 reconnaissance, which is why this is filed rather than done: **the eighteen cards do not fail for one reason, they fail for several, and the existing machinery already covers more of them than it looks.** `mass_mind_control` ("Mark two enemy pieces of any type below queen. After your opponent's next move, any of them still in place defect to your side") is two sentences with a nounless pronoun-subject second clause, which is EXACTLY the shape `carried` already handles, so it is failing on something narrower (probably "defect" missing from the GAIN verb set, or `PRONOUN_SUBJECT` not matching "any of them"). `lightning_strike` is one sentence with a semicolon, so `carried` never sees it, and its later clause DOES contain a noun ("each marked piece"), so the `!hasNoun` test excludes it too: that one needs same-sentence anaphora with an explicit back-reference marker ("marked"). Diagnose each card individually before writing anything general, and expect to fix three or four small things rather than one big one. Two second-order shapes ride along: a piece-class upgrade in place ("it becomes a queen where it stands", "any two of your pieces become queens") is worth the DIFFERENCE, and a defection ("defect to your side for the game") is worth DOUBLE, since the piece leaves one army and joins the other | M | TODO |
...
| B9 | **Draft cards carry no selection state for a screen reader.** The card buttons have no `aria-pressed` and no `aria-selected`; the only signal that a card is chosen is the commit button renaming itself from "Pick a card" to "Confirm <name>". Visually the selection is obvious, and to a screen reader it is a button that did nothing. Located: `DraftOverlay.tsx` already passes `glow={selected === i}` down to the card at both call sites (the full overlay and the compact panel), so the state is there and simply never reaches ARIA. The `<button>` itself is rendered inside `BuffCard.tsx` / `NerfCard.tsx`, which is where `aria-pressed` belongs | S | TODO |
| B10 | **Accessible names and the live region run words together.** A draft card announces as `"Walking Pace, PleaseMovementPassiveITrivialOnce, your a-file or b-file pa..."`: the name, category, kind, tier numeral and rarity badges are concatenated with no separator. The board's live region does the same across whole sentences: `"black knight g8 to f6 | Special OrderI | Special OrderIBot played a buffYour next draft is dealt from tier 2."` Both are badge spans with no whitespace or punctuation between them. Located: the run is name + category + kind + tier numeral + rarity + description, all rendered as adjacent spans inside `BuffCard.tsx` / `NerfCard.tsx`, so one `sr-only` separator at the badge level fixes every surface that shows a card at once. Do it in the same pass as B9, which is in the same two files | S | TODO |
...
| B12 | **Nearly a third of the early game is spent unable to touch the board.** A whole-game harness (`e2e/feel.spec.ts`, "a whole game, start to result") plays real moves and times the arc: **3 drafts costing 13.0s across a 43s session (30%)** on one run and **5 drafts costing 24.4s across 94s (26%)** on a longer one, averaging 4.3 to 4.9s each. Two runs, two sessions, the same quarter to a third. B8 measured one draft in isolation; this is the cumulative number, and it is the one that decides whether a session feels good. Fixing B8 fixes this | S | TODO |
| B13 | **The draft overlay and the game-over panel are both bare `role="dialog"` with nothing to tell them apart.** To a screen reader they are the same kind of thing, and to anything automating the page they are indistinguishable: the whole-game harness mistook the ending for a draft and waited ten minutes for cards that would never deal. Each needs an `aria-label` (or `aria-labelledby` pointing at its own title, which game-over already has as `#game-over-title`) | XS | TODO |
...
| C39 | **The light theme's accent "hi" is LIGHTER than its base, which is the wrong direction for paper.** `applyUiPrefs` writes `SITE_THEMES.light.accent.rgbHi` as an inline style on `<html>`, and light's hi is `#3692e7` against a base of `#1b78d0`. The header "Sign in" link is `text-gold-leaf` mapped to `--accent-hi-rgb` and measures **2.75:1** across 21 rendered elements. It cannot be fixed from CSS at all: the value is inline on the root. Light's hi needs to be a step DARKER, around `#14589f`. `src/lib/settings.ts` | S | TODO |
| C40 | **Tier and rarity chips are the largest remaining contrast block, and they need an owner rather than a unilateral edit.** Dark tier-8 `#e05252` on its `.tier-bg-8` wash is **3.26:1** across 32 elements on `/codex`; light tier-9 is **1.42:1** across 18; light tier-1 2.17, tier-7 2.75. `/achievements` rarity chips in light: Common **1.69**, Legendary **1.58**, Rare 2.04, Epic 3.01. All 12px, so 4.5:1 applies and none reaches the large-text exemption. Two possible fixes, a per-theme tier palette (10 new hexes) or a change to the `.tier-bg-*` fill alphas, and `docs/design-system.md` pins tier colour explicitly ("Card tiers everywhere, no exceptions"). Getting it wrong is visible on every card in the game | M | TODO |
| C41 | **`ModShell.tsx` is the whole of the remaining crawler-visible sub-12px text.** Six 11px rail labels (`Ctrl K`, Queue, People, Activity, Cards, Site) repeated across four `/mod` routes are 24 of 24 rendered violations per theme. Raising them takes the measured sitewide count to **0**. Lines 74, 80, 156, 268, 284. Behind it, 26 more 11px sites across `src/components/mod/**` and three 10px sites in `Board.tsx` (467, 471, 537) which are not crawler-reachable | S | TODO |
| C42 | `src/components/effects/creatorPlays.css` has five rules at 8 to 10px (`.cpl-*-caption`, `.cpl-*-split`). Real DOM text under the absolute floor, inside a transient card-play stage: percentage-positioned `white-space: nowrap` captions, so a 1.5x bump needs a per-scene layout pass and each scene has to be triggered to verify it. (`useSpectacle.css`'s six `font-size: 4px` rules are NOT a violation: no text, the `em` unit feeds `box-shadow` offsets on a particle.) | S | TODO |
| C43 | **28 touch-target findings remain after round 7, on three routes.** `/tournaments/[id]` 12 and `/clubs/[slug]` 8: a breadcrumb "Tournaments" link at 18px, a club member row (`li > a.flex.items-center.gap-2.5`), and a player-name link that is NOT `PlayerLink` (it spells `a.hover:text-gold-leaf` directly, so the round-7 `PlayerLink` fix does not reach it). `/tutorial/first-game` 8, and that route is **flaky by nature**: its findings differ run to run because the game state does ("Recent plays1", "Dismiss Layaway Plan" appeared on one run and not the next), so it needs a seeded position before its count means anything | S | TODO |
| C44 | **The sweep baseline is a valid ceiling but is now loose.** It was written mid-round, before the last four fixes (the move strip, `PlayerLink`, the `/u/[username]` achievements button, the `/tv` and `/analysis` and `/achievements` controls), so the current tree measures below it on several routes. Shrink-only means that is safe: it cannot false-alarm, it just will not catch a small regression on those pairs. Re-run `SWEEP_FORCE=1 SWEEP_WRITE_BASELINE=1` on a quiet tree to lock the gains in | XS | TODO |
