...
| C53 | **`DraftResolvingChip` claims "Resolving effects" for the whole ~330ms opening gap when nothing is resolving.** It appears in the same frame as the board and disappears in the same frame as the dialog, every run, and at game start there is nothing to resolve. Gating it on `sigBusy` would make it honest. Behaviour deliberately left alone in round 9 because C49's own fix changes that window. **Round 10: C49 landed and the chip is now painted in 0/5 measured runs at game start** -- it is unreachable on a quiet board rather than corrected, so the dishonest wording still stands for the busy-board case | XS | TODO |
