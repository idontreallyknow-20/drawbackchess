...
| A6 | `amazon_army` t7 measured -25 points, the same family as A11 and possibly the same cause. The activation policy now simulates before it picks, so RE-MEASURE before investigating further: the number may simply be gone | S | TODO (re-measure first) |
